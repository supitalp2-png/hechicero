...
- [ ] TICKET-102 — bug — Écran de veille et coupure d'écran ne fonctionnaient plus après l'intégration hardware finale (2026-07-08) — **récidive le 2026-07-08 (soir), cause différente, toujours pas résolu définitivement**
      ...
      - ✅ **CAUSE TROUVÉE ET CORRIGÉE (2026-07-09)**, confirmée par `sleep_debug.log` : `checkParentalTime()` tourne toutes les 30s (`setInterval`, vérif horaires parentaux) et appelait `loadParentalConfig()` → `applySleepConfig()` → **`resetSleepTimer()` à chaque fois, inconditionnellement** — même quand rien n'avait changé. Le log a montré `reset_timer source=config_reload ms_since_last_reset=30000` en boucle avec `delay_ms=120000` : dès que `sleep_delay` (config admin) dépassait 30s, ce refresh périodique repoussait perpétuellement le timer d'inactivité avant qu'il n'ait la moindre chance d'atteindre son délai — l'overlay ne pouvait alors **jamais** se déclencher naturellement, peu importe l'inactivité réelle. C'est le bug structurel derrière toutes les récidives.
      - **Fix** : `applySleepConfig()` compare désormais la config reçue à la dernière appliquée (`SLEEP_ENABLED`/`SLEEP_DELAY`/`SLEEP_MODE` + flag `sleepConfigInitialized`) et n'appelle `resetSleepTimer()` que si la config a réellement changé (ou 1er chargement) — un refresh qui ne change rien ne touche plus au timer d'inactivité.
      - ⚠️ Le fichier `web/lecteur/index.html` a été tronqué lors d'une écriture directe sur Q:\ (bug déjà connu, voir `feedback_write_tool_truncation.md`) — récupéré en repartant du dernier commit (`git show HEAD:...`, non corrompu) + ré-application du fix via un script, écriture finale faite en bash (pas via l'outil d'édition Windows) pour éviter une nouvelle troncature. Fichier revérifié intact (taille + syntaxe JS + balises finales) après coup.
      - ⚠️ **Découverte annexe pendant ce diagnostic** : `git log`/`git show` échouent sur le dépôt avec `error: garbage at end of loose object ... fatal: ... is corrupt` (objet `4236ac6e...`). Un objet du dépôt git est corrompu. `git show HEAD:<fichier>` fonctionne malgré tout (l'objet corrompu n'est pas le blob HEAD des fichiers touchés ici), mais `git log`/`git fsck --full` échouent ou n'aboutissent pas. **À investiguer par Thomas en SSH** sur le Pi (pas sûr que ce soit lié à Samba puisque les commits se font en SSH natif) — voir si ça bloque un futur commit/push avant de continuer à committer sur ce dépôt.
      - ⏳ Reste à faire : Thomas recharge/relance le kiosk et confirme que la veille se déclenche à nouveau normalement ; committer ce fix ; investiguer l'objet git corrompu
