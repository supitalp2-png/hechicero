...
- [ ] TICKET-046 — UX — Favoris (cœur) accessibles rapidement
      - 🔍 **Cadrage en cours (2026-07-19)**, décision avant dev. Existant côté matériel : GPIO23 (bouton isolé, emplacement antenne) déjà câblé et réservé pour ce bouton depuis TICKET-101 (2026-07-08), branché en `handle_unassigned` dans `buttons_daemon.py` en attendant ce ticket — voir [[project_hechicero_buttons_gpio]].
      ...
      - Aucune structure de données favoris n'existe aujourd'hui (`data.json` n'a pas de champ favori, et il est régénéré par l'ingestion RSS — donc pas un bon endroit pour stocker un choix persistant de l'enfant).
      - Référence externe (Merlin, l'enceinte que le fils de Thomas utilise aujourd'hui) : bouton ♥ physique avec 3 usages — 1) pendant la navigation, ouvre directement la liste des titres favoris ; 2) pendant l'écoute, ajoute/retire le titre en cours (cœur bleu affiché sur la jaquette) ; 3) appui long = batterie + date/heure (fonction annexe, sans rapport direct avec les favoris).
      - ✅ **Cahier des charges figé (2026-07-19)**, dev pas encore commencé (Thomas manque de temps pour l'instant) :
        - Portée : favori par **épisode**, pas par podcast entier (confirmé avec l'enfant).
        - Déclencheur : bouton physique dédié = **GPIO16** (confirmé sur le boîtier réel le 2026-07-19 via `buttons_daemon.py` en mode identification — dernier bouton de la ligne des 7, jusque-là en réserve). GPIO23 (bouton isolé antenne) réservé pour un usage futur différent, pas le favori. Cf. [[project_hechicero_buttons_gpio]].
        - Tap court = ajoute/retire le favori sur l'épisode en cours d'écoute.
        - Appui long = ouvre un écran dédié listant les favoris (façon Merlin), confirmé avec l'enfant.
        - Retour visuel à l'ajout : un cœur apparaît et se fixe sur la jaquette/icône de l'épisode. **Pas de son** (confirmé).
        - Sortie de l'écran favoris : prévoir un moyen simple de sortir (mécanisme exact à définir au moment du dev — cohérent avec la nav sans texte de l'IHM enfant).
      ...
      - 🛠️ **Implémenté le 2026-07-19, pas encore testé en conditions réelles.** Détail :
        - `data/favoris.json` (nouveau, hors dépôt comme les autres fichiers d'état runtime) : dict clé `podcastId/episodeId` → `{podcast_id, episode_id, added_at}`.
        - `web/lecteur/radio.php` : nouvelles actions `toggle_favori` (bouton physique, résout l'épisode en cours via MPD comme next_episode/prev_episode), `get_favoris` (liste enrichie, admin + écran tactile), `remove_favori` (admin), `request_screen`/`get_ui_request` (mécanisme polling pour que l'appui long ouvre l'écran favoris côté tactile — pas de canal direct entre le daemon Python des boutons et le navigateur).
        - `scripts/buttons_daemon.py` : GPIO16 rejoint `TAP_OR_HOLD` (tap = `handle_favori_toggle`, maintien = `handle_favori_screen`), même mécanique que next/précédent.
        - `web/lecteur/index.html` : nouvel écran `#favoris` (retour simple vers l'accueil, même pattern que les autres écrans), badge cœur (`.fav-heart`/`.is-fav`) sur la jaquette de l'écran lecteur et sur les vignettes de la liste d'épisodes, deux boucles de polling globales (indépendantes de l'écran affiché) : `pollUiRequest` (1s, détecte l'appui long) et `fetchFavoris` (4s, rafraîchit les cœurs déjà affichés).
        - `web/admin/favoris.php` (nouveau) : liste des favoris avec vignette/titre/podcast/date, bouton "Retirer". Lien nav "❤️ Favoris" ajouté sur `index.php`, `dashboard.php`, `battery_dashboard.php`, `audio_eq.php`.
        - `.gitignore` : `data/favoris.json` et `data/ui_request.json` ajoutés (fichiers runtime, même traitement que `audio_eq.json`).
      - 🛠️ **Retours Thomas après 1er essai réel (2026-07-19)**, tous traités :
        - Cœur trop petit + pas d'animation → agrandi (46px vignettes, 68px écran lecteur) + animation `favPop` (rebond avec légère rotation, 600ms, ne rejoue que quand `.is-fav` passe de absent à présent).
        - Apparition trop lente → `fetchFavoris` resserré de 4s à 1s (le bouton physique ne notifie pas directement le navigateur, seul le polling permet de le savoir — 1s = meilleur compromis réactivité/charge).
        - Retrait par second appui : déjà correct dans `toggle_favori` (add/remove symétriques sur la même clé), confirmé en relisant le code pendant l'ajout des webradios ci-dessous — pas de fix nécessaire.
        - **Webradios favorisables** : `favori_key()` désormais préfixé par type (`episode:podcastId/episodeId` ou `radio:radioId`, la clé vient du serveur — plus de reconstruction côté client). `toggle_favori` détecte une webradio via `is_webradio_uri()` + nouvelle `find_current_radio()` (match par URL). `get_favoris`/admin enrichissent aussi les favoris radio (`type:'radio'`, image/nom de la station). Cœur ajouté sur le catalogue radio (`.radio-row`) et l'écran lecteur radio (`#rp-art-box`, nouvel id).
      - 🛠️ **Navigation au sein des favoris (2026-07-19)** : suivant/précédent, quand on lance un épisode ou une webradio depuis l'écran favoris, naviguent désormais dans CETTE liste plutôt que dans le podcast d'origine. Implémenté **côté boutons à l'écran uniquement** (pas les boutons physiques GPIO17/27, qui gardent leur scope podcast actuel — étendre ça au physique demanderait un contexte de navigation côté serveur + une resynchro `now_playing` consciente des webradios, pas fait, à discuter si Thomas le veut).
        - `favNavQueue`/`favNavIdx` (JS) : snapshot de la liste favoris au moment du clic, remis à `null` dès qu'on relance depuis un écran normal (chapitres, catalogue radio, reprise batterie).
        - `playFavItem()` factorise le branchement épisode/webradio, réutilisé au clic initial et à chaque suivant/précédent.
        - Nouveaux boutons ⏮/⏭ sur l'écran lecteur radio (`#rp-btn-prev`/`#rp-btn-next`, inexistants avant — une webradio browsée normalement n'a pas de suivant/précédent), cachés (`hidden`) sauf en navigation favoris.
      - 🛠️ **Parité bouton physique (2026-07-19, même soir)** : demande de Thomas — le suivant/précédent physique (GPIO17/27) doit aussi naviguer dans les favoris, pas seulement les boutons à l'écran. Le bouton physique n'a aucun canal direct vers le navigateur (process Python séparé), donc contexte partagé côté serveur :
        - `data/nav_context.json` (nouveau) : `{mode, keys[], active_key}`, posé/effacé par une nouvelle action `set_nav_context` (appelée par index.html à chaque changement de contexte — clic sur l'écran favoris, suivant/précédent, retour à un écran classique).
        - `next_episode`/`prev_episode` consultent ce contexte en premier : si `mode=favoris`, naviguent dans `keys` via une nouvelle `resolve_favori_key()` (gère épisode ET webradio) au lieu du scope podcast historique — inchangé si `mode=normal`. **`scripts/buttons_daemon.py` n'a pas eu besoin d'être modifié** : le bouton physique appelait déjà ces mêmes actions, toute l'intelligence est côté serveur.
        - `now_playing` détecte désormais aussi une webradio en cours (avant : épisodes de podcast uniquement) et renvoie le contexte de navigation courant — nécessaire pour que l'IHM se resynchronise correctement quand c'est le bouton physique qui a changé de piste pendant que l'enfant regarde l'écran (`syncFavNavFromServer()`, `syncRadioDisplay()` côté JS, écran basculé automatiquement seulement si déjà sur un des deux écrans de lecture — jamais depuis un écran de navigation).
      - ⏳ **Reste à faire** : re-tester sur le Pi (rien à déployer côté fichiers pour radio.php/index.html — juste recharger la page lecteur/kiosk ; `buttons_daemon.service` pas besoin d'être redémarré cette fois) : cœur (taille/animation/délai), favoris webradio, retrait par second appui, navigation suivant/précédent au sein des favoris à l'écran ET au bouton physique (épisode→épisode, épisode→radio, radio→radio), resynchro de l'écran quand le bouton physique change de type de contenu.
