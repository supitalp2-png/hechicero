...
- [~] TICKET-130 — bug/données — Neuf podcasts ont disparu de la config, en silence, pendant deux semaines (2026-08-17)
      ...
      - ⏳ **Reste** : lancer la restauration, puis `ingest.py` pour reconstruire `data.json`. Et **`data/parental.json` court exactement le même risque** — même statut, même IHM, aucun garde-fou. À traiter.
